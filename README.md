...
| **`tuition-service`** | Python 3.11 + FastAPI + Motor | `8732` | Student tuition ledger & invoice settlement |
...
| **MongoDB** | MongoDB 7.0 | `27017` | Multi-tenant logical isolated databases |
...
1. **Atomic Conditional Update:**
   - Account Balance Deduction: Utilizes MongoDB `find_one_and_update` with condition `{"balance": {"$gte": amount}}`. If concurrent transactions arrive simultaneously, only one succeeds; the other receives `None` and is rejected immediately.
   - Tuition Invoice Settlement: Utilizes MongoDB `find_one_and_update` with condition `{"status": "UNPAID"}` directly at the database engine level.
...
3. **Two-Factor OTP Token Lifecycle:**
   - 6-digit OTP tokens are persisted directly in MongoDB with a 5-minute TTL Index for automated document expiry.
...
Data is automatically seeded from `init-db/init-mongo.js` on the initial startup of the MongoDB container.
